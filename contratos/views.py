...
def cadastro_contrato(request):
    ...
    if usuario:
        if request.method == 'GET':    
            ...
            produto = request.POST.getlist("select_produto")
            valor = request.POST.getlist("campo_valor")    
            quantidade = request.POST.getlist("quantidade")
            unid_medida = request.POST.getlist("unid_medida")
            
            
            
            for cod_produto,vl_item,quantia,unid_med  in zip(produto,valor,quantidade,unid_medida): # quantidade,quantia
...
def update(request,seq_contrato):
    ...
    if usuario:
        if request.method == 'GET':
            ...
            cursor = connection.cursor()
            cursor.execute(f'''
            select * from ek_contrato_detalhe where seq_contrato = {seq_contrato} order by seq_contrato_detalhe
            ''')
            resultado = cursor.fetchall()
        ...
        if request.method == 'POST':
            ...
            quantidade = request.POST.getlist('quantidade')
            unid_medidas = request.POST.getlist('unid_medida')
            seq_detalhe = request.POST.getlist('sequencial_item')
            print("Veio do template: ",seq_detalhe)
            ...
            lista_sequencia_detalhe = [ str(seq[0]) for seq in sequencial_item]
            print('Veio do banco: ' , lista_sequencia_detalhe)

            itens_deletar = [item for item in lista_sequencia_detalhe if item not in seq_detalhe ]
            print("Deletar: " , itens_deletar)
            ...
            for item in itens_deletar:
                ...
                cursor.execute(
                    ...
                    where seq_contrato = {seq_contrato} and seq_contrato_detalhe = {item}
                    '''
                )
            ...
            for seq_item_contrato,(produto , valor,quantia ,unid_medida,sequencial_detalhe) in enumerate(zip(produto,valores,quantidade,unid_medidas,seq_detalhe)):
                print('Dentro do for', str(sequencial_detalhe))
                
                
                cursor.execute("select max(seq_contrato_detalhe) from ek_contrato_detalhe ")
                max_seq_detalhe = cursor.fetchall()[0][0] + 1
                
                print(f'''
                        Prod: {produto},
                        Valor : {valores},
                        Quantidade: {quantidade},
                        Unidade: {unid_medidas}''')

                descricao = EkProduto.objects.get(cod_produto = produto ).desc_produto
                
                if sequencial_detalhe == 'new':
                    cursor.execute(f"""
                            ...
                            VALUES ({max_seq_detalhe}, {seq_contrato}, {produto}, '{descricao}', {valor}, {quantia}, '{unid_medida}');""")
                else:
                    ...
                    cursor.execute(
                        f'''
                            ...
                            where seq_contrato_detalhe = {lista_sequencia_detalhe[seq_item_contrato]}
                        '''
                    )
    ...
    if usuario:
        ...
        if request.POST:
            if atributos[0] == 0 and atributos[1] == 0:
                ...
                connection.commit()
                return redirect('/home/?status=14')
            else:
                return redirect('/home/?status=2')
        return render(request,'contratos/fechar.html',{'seq_contrato':seq_contrato})
    else:
        return redirect('/login/?status=2')